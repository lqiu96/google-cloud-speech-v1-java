...
public class AdaptationClient implements BackgroundResource {
   ...
   *   for (Operation element : adaptationClient.listOperations(name, filter).iterateAll()) {
   *     // doThingsWith(element);
   *   }
   * }
   * }</pre>
   ...
   *           .setPageToken("pageToken873572522")
   *           .build();
   *   for (Operation element : adaptationClient.listOperations(request).iterateAll()) {
   *     // doThingsWith(element);
   *   }
   * }
   * }</pre>
   ...
   *       adaptationClient.listOperationsPagedCallable().futureCall(request);
   *   // Do something.
   ...
   *           .build();
   *   while (true) {
   *     ListOperationsResponse response = adaptationClient.listOperationsCallable().call(request);
   ...
   * try (AdaptationClient adaptationClient = AdaptationClient.create()) {
   *   String name = "name3373707";
   *   Operation response = adaptationClient.getOperation(name);
   * }
   * }</pre>
   *
   ...
   *   Operation response = adaptationClient.getOperation(request);
   * }
   * }</pre>
   *
   * @param request The request object containing all of the parameters for the API call.
   ...
   * try (AdaptationClient adaptationClient = AdaptationClient.create()) {
   *   GetOperationRequest request = GetOperationRequest.newBuilder().setName("name3373707").build();
   *   ApiFuture<Operation> future = adaptationClient.getOperationCallable().futureCall(request);
   *   // Do something.
